...
class ReactVlcPlayerView extends TextureView implements
    ...
    public void onHostResume() {
        if (mMediaPlayer == null || !isHostPaused) return;
    ...
    private void attachVideoSurface() {
        if (mMediaPlayer == null || getSurfaceTexture() == null) return;
    ...
    private void createPlayer(boolean autoplayResume, boolean isResume) {
        ...
        try {
            ...
            DisplayMetrics dm = getResources().getDisplayMetrics();
            Media m = null;
            if (isNetwork || (uriString != null && (uriString.startsWith("file://") || uriString.startsWith("content://")))) {
    ...
    public void setSrc(ReadableMap src) {
        ...
        if (mMediaPlayer != null && srcMap != null && srcMap.hasKey("uri") && !srcMap.isNull("uri")
                && nextUri != null && nextUri.equals(srcMap.getString("uri"))) {
    ...
    public void setPausedModifier(boolean paused) {
        ...
        if (mMediaPlayer != null) {
            if (paused) {
                ...
                isPaused = false;
                if (playInBackground) acquireWakeLock();
                mMediaPlayer.play();
                Log.i("do play:", true + "");
            }
        } else {
    ...
    public void setAspectRatio(String aspectRatio) {
        if (mMediaPlayer != null) {
            if (aspectRatio == null || aspectRatio.length() == 0 || "FIT".equalsIgnoreCase(aspectRatio) || "FIT_SCREEN".equalsIgnoreCase(aspectRatio)) {
